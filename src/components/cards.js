import React, { useEffect } from 'react';
import { getDistanceBetweenCoordinates } from '../util/helper';
import './cards.css';
function Cards({ lat, long, handleExperienceClick }) {

    const getProximity = (thislat, thislong) => {
        if (lat === null || long === null) {
            return false; 
        }
        let distance = getDistanceBetweenCoordinates(lat, long, thislat, thislong);
        return distance;
    }
    const experienceClick = (event) => {
        let experienceId = event.target.getAttribute('data-experienceid');
        let experienceName = event.target.getAttribute('data-experiencename');
        handleExperienceClick(experienceId, experienceName);
        
    }

    return (
        <div className="cards-container">
            <div onClick={experienceClick} data-experiencename={"Halloween på DOKK1"} data-experienceid={1} className="flex m-5 border border-gray-900 justify-center">
                <div className="h-30 h-auto w-30 flex-none bg-cover rounded-t rounded-t-none rounded-l text-center overflow-hidden pointer-events-none" title="test">
                </div>
                <div className="py-20 flex flex-col justify-between leading-normal pointer-events-none">
                    <div >
                        <div className="font-bold text-lg text-white pointer-events-none">Halloween på DOKK1</div>
                        {getProximity(56.153249691363776, 10.21373525598003) && <span className="text-white">{getProximity(56.153249691363776, 10.214202802135777)} Meter</span>}
                    </div>
                </div>
            </div>

            <div className="flex m-5 grayscale justify-center">
                <div className="h-30 h-auto w-30 flex-none bg-cover rounded-t rounded-t-none rounded-l text-center overflow-hidden pointer-events-none" title="test">
                </div>
                <div className="py-20 flex flex-col justify-between leading-normal pointer-events-none">
                    <div >
                        <div className="font-bold text-lg text-white pointer-events-none flex justify-center">Eventyr i dyrehaven <img className="inline-block h-5 my-auto ml-1" src="data:image/png;base64," /></div>
                        {getProximity(56.12287382586223, 10.218102110686793) && <span className="text-white">{getProximity(56.12287382586223, 10.218102110686793)} Meter</span>}
                    </div>
                </div>
            </div>

            <div className="flex m-5 grayscale justify-center">
                <div className="h-30 h-auto w-30 flex-none bg-cover rounded-t rounded-t-none rounded-l text-center overflow-hidden pointer-events-none" title="test">
                </div>
                <div className=" py-20 flex flex-col justify-between leading-normal pointer-events-none">
                    <div >
                        <div className="font-bold text-lg text-white pointer-events-none flex justify-center">Fortællinger om domkirken <img className="inline-block h-5 my-auto ml-1" src="data:image/png;base64," /></div>
                        {getProximity(56.156999232502145, 10.21090265635068) && <span className="text-white">{getProximity(56.156999232502145, 10.21090265635068)} Meter</span>}
                    </div>
                </div>
            </div>
        </div>
    );
}
export default Cards;