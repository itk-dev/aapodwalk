...
export function getIndexedPinSvg(index) {
  return `<div class="relative w-8 h-8">
            <svg aria-hidden="true" width="32" height="32" viewBox="0 0 32 33" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="pin-label"><span class="sr-only">Dette punkt er nummer ${index} i ruten </span>${index}</div>
          </div>`;
}
...
export function getPinSvg() {
  return `<div>
            ...
            <span class="sr-only">Punktet på kortet</span>
          </div>`;
}
...
export function getYouAreHerePin() {
  return `<div><span class="sr-only">Her er du</span>${YouAreHere}</div>`;
}
